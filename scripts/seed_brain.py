...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed the Second Brain with cinematic demo thoughts.")
    parser.add_argument("--api", default="http://localhost:8000", help="FastAPI base URL")
    parser.add_argument("--limit", type=int, default=None, help="Optional limit on number of thoughts")
    args = parser.parse_args()

    base = args.api.rstrip("/")
    url = f"{base}/ingest/thought"
    thoughts = SEED_THOUGHTS[: args.limit] if args.limit else SEED_THOUGHTS

    print(f"Seeding {len(thoughts)} thoughts to {url}")
    for idx, thought in enumerate(thoughts, 1):
        try:
            res = requests.post(url, json=thought, timeout=20)
            if res.ok:
                preview = shorten(thought["content"], width=60, placeholder="...")
                print(f"[{idx:02d}] OK  {preview}")
            else:
                print(f"[{idx:02d}] FAIL {res.status_code} {res.text}")
